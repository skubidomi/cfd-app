...
void writedatafiles(double *psi, int m, int n, int scale)
{
  ...
  for (int i=0;i<m;i++) {
    for (int j=0;j<n;j++) {
     vel[i*n+j][0] =  (psi[(i+1)*(n+2)+j+2]-psi[(i+1)*(n+2)+j])/2.0;
     vel[i*n+j][1] = -(psi[(i+2)*(n+2)+j+1]-psi[i*(n+2)+j+1])/2.0;
     ...
     hue2rgb(hue,&(rgb[i*n+j][0]),&(rgb[i*n+j][1]),&(rgb[i*n+j][2]));
    }
  }
  ...
  for (int i=0;i<m;i++) {
    ...
    for (int j=0;j<n;j++) {
      ...
      fprintf(cfile,"%i %i %i %i %i\n", ix, iy,
        rgb[i*n+j][0], rgb[i*n+j][1],rgb[i*n+j][2]);

      if ((ix-1)%scale == (scale-1)/2 &&
       (iy-1)%scale == (scale-1)/2    )
      {
       fprintf(vfile,"%i %i %f %f\n",
        ix,iy,vel[i*n+j][0],vel[i*n+j][1]);
      }
    }
  }
...
void writeplotfile(int m, int n, int scale)
{
  ...
  fprintf(gnuplot,"set output \"out.png\"\n");
